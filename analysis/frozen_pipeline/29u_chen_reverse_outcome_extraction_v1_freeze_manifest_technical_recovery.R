...
if (length(args) == 2L && identical(args[[1L]], "--project-root")) {
  ...
  stop("Usage: Rscript R/29u_chen_reverse_outcome_extraction_v1_freeze_manifest_technical_recovery.R [--project-root <path>]", call. = FALSE)
}
setwd(root)
...
add_manifest <- function(category, role, path) {
  data.frame(category = category, role = role, path = path, stringsAsFactors = FALSE)
}

paths <- list(
  script_failed = rel("R", "29t_chen_reverse_outcome_extraction_v1_freeze_manifest.R"),
  script = rel("R", "29u_chen_reverse_outcome_extraction_v1_freeze_manifest_technical_recovery.R"),
  ...
  initial_extraction_script = rel("R", "29_chen_reverse_outcome_extraction_v1.R"),
...
inputs <- unlist(paths[c(
  "script_failed", "script", "decision109_qc", "target_authority", "source_cert", "chen_raw",
  "initial_extraction_script", "extraction_script", "readback_script", "extraction_qc",
  "readback_qc", "readback_mismatch", "initial_failed_log", "extraction_recovery_log",
  "readback_log", "decision110", "decision111", "union_targets", "master_parquet",
  "master_tsv", "strict_inc_parquet", "strict_inc_tsv", "strict_exc_parquet",
...
log_line("stage=chen_reverse_outcome_extraction_v1_freeze_technical_recovery_start")
...
manifest_items <- do.call(rbind, list(
  add_manifest("script", "initial_failed_extraction_script", paths$initial_extraction_script),
  add_manifest("script", "technical_recovery_extraction_script", paths$extraction_script),
  add_manifest("script", "readback_recovery_script", paths$readback_script),
  add_manifest("script", "failed_freeze_script_preserved", paths$script_failed),
  add_manifest("script", "freeze_manifest_technical_recovery_script", paths$script),
  add_manifest("authority", "decision_109_contract", paths$decision109_qc),
  add_manifest("authority", "corrected_target_authority", paths$target_authority),
  add_manifest("authority", "chen_source_certification", paths$source_cert),
  add_manifest("raw_source", "chen_raw_source_read_only", paths$chen_raw),
  add_manifest("qc", "decision_110_extraction_qc_failed_preserved", paths$extraction_qc),
  add_manifest("qc", "decision_111_readback_recovery_qc", paths$readback_qc),
  add_manifest("qc", "decision_111_mismatch_audit", paths$readback_mismatch),
  add_manifest("qc", "match_audit", paths$match_audit),
  add_manifest("qc", "missing_audit", paths$missing),
  add_manifest("log", "initial_failed_scan_log", paths$initial_failed_log),
  add_manifest("log", "technical_recovery_extraction_log", paths$extraction_recovery_log),
  add_manifest("log", "readback_recovery_log", paths$readback_log),
  add_manifest("decision", "decision_110_extraction", paths$decision110),
  add_manifest("decision", "decision_111_readback_recovery", paths$decision111),
  add_manifest("data", "union_targets", paths$union_targets),
  add_manifest("data", "master_parquet", paths$master_parquet),
  add_manifest("data", "master_tsv", paths$master_tsv),
  add_manifest("data", "strict_included_parquet", paths$strict_inc_parquet),
  add_manifest("data", "strict_included_tsv", paths$strict_inc_tsv),
  add_manifest("data", "strict_excluded_parquet", paths$strict_exc_parquet),
  add_manifest("data", "strict_excluded_tsv", paths$strict_exc_tsv),
  add_manifest("data", "relaxed_included_parquet", paths$relaxed_inc_parquet),
  add_manifest("data", "relaxed_included_tsv", paths$relaxed_inc_tsv),
  add_manifest("data", "relaxed_excluded_parquet", paths$relaxed_exc_parquet),
  add_manifest("data", "relaxed_excluded_tsv", paths$relaxed_exc_tsv),
  add_manifest("environment", "renv_lock", paths$renv_lock)
))
...
manifest_items$sha256 <- vapply(manifest_items$path, hash_file, character(1))

write_csv(manifest_items, paths$manifest_csv)
manifest_sha256 <- hash_file(paths$manifest_csv)

multiple_file <- rel("results", "qc", "chen_reverse_outcome_multiple_matches_v1.tsv")
hard_checks <- list(
  ...
  source_rows_scanned_once_in_successful_extraction = identical(as.integer(extraction_qc$source_rows_scanned), 50638016L),
  ...
  no_multiple_match_file_created = !file.exists(multiple_file),
  no_incompatible_targets = identical(as.integer(extraction_qc$union_marker_effect_allele_incompatible_count), 0L),
  ...
  all_manifest_inputs_exist = all(manifest_items$exists),
  manifest_sha256_created = nzchar(manifest_sha256),
  failed_freeze_script_preserved = file.exists(paths$script_failed),
...
freeze <- list(
  ...
  informational_findings = list(
    ...
    failed_freeze_script_preserved = relpath(paths$script_failed),
    freeze_recovery_script = relpath(paths$script),
...
decision_lines <- c(
  ...
  "The first freeze script failed before writing outputs because of a syntax error. This technical recovery preserves that script and changes only the freeze-manifest implementation.",
  "",
  "## Frozen Evidence",
