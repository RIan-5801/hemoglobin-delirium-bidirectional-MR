...
if (length(args) == 2L && identical(args[[1L]], "--project-root")) {
  ...
  stop("Usage: Rscript R/29r_chen_reverse_outcome_extraction_v1_technical_recovery.R [--project-root <path>]", call. = FALSE)
}
setwd(root)
...
marker_parts <- function(marker) {
  z <- strsplit(as.character(marker), "[:_]", perl = TRUE)[[1L]]
  if (length(z) != 4L) return(rep(NA_character_, 4L))
  z
}
get_chr <- function(row, nm) {
  if (is.null(row) || !(nm %in% names(row)) || length(row[[nm]]) < 1L) return(NA_character_)
  val <- row[[nm]][[1L]]
  if (is.null(val) || length(val) == 0L) NA_character_ else as.character(val)
}
get_num <- function(row, nm) parse_num(get_chr(row, nm))

paths <- list(
  failed_script = rel("R", "29_chen_reverse_outcome_extraction_v1.R"),
  recovery_script = rel("R", "29r_chen_reverse_outcome_extraction_v1_technical_recovery.R"),
  ...
  failed_log = rel("results", "logs", "chen_reverse_outcome_extraction_v1.log"),
  recovery_log = rel("results", "logs", "chen_reverse_outcome_extraction_v1_technical_recovery.log"),
...
stop_if(!file.exists(paths$failed_log), "Expected preserved failed log from initial Decision 110 attempt is absent.")
...
log_line <- function(...) cat(sprintf("[%s] %s\n", ts(), paste0(..., collapse = "")), file = paths$recovery_log, append = TRUE)
log_line("stage=chen_reverse_outcome_extraction_v1_technical_recovery_start")
...
required_target_cols <- c(
  ...
  "expected_chen_marker_id", "identity_bridge_ready", "reference_chr_grch37", "reference_pos_grch37",
  "reference_allele1", "reference_allele2", "exposure_effect_allele", "exposure_other_allele"
)
...
for (x in c("strict_included_member", "strict_excluded_member", "relaxed_included_member", "relaxed_excluded_member", "identity_bridge_ready")) {
  target_authority[[x]] <- bool(target_authority[[x]])
}
...
header <- sub("\r$", "", readLines(header_con, n = 1L, warn = FALSE))
close(header_con)
...
repeat {
  lines <- sub("\r$", "", readLines(con, n = 100000L, warn = FALSE))
...
match_df <- if (length(matches) == 0L) {
  ...
  as.data.frame(do.call(rbind, lapply(matches, function(z) {
    z <- as.character(z)
    names(z) <- header_fields
    z
...
master_rows <- lapply(seq_len(nrow(target_authority)), function(i) {
  ...
  marker <- target$expected_chen_marker_id[[1L]]
  mp <- marker_parts(marker)
  rows <- match_df[match_df$rs_number == marker, , drop = FALSE]
  raw_count <- nrow(rows)
  status <- if (!isTRUE(target$identity_bridge_ready[[1L]])) {
  ...
  data.frame(
    target_rsid = target$rsid[[1L]],
    expected_chen_marker_id = marker,
    strict_included_member = isTRUE(target$strict_included_member[[1L]]),
    strict_excluded_member = isTRUE(target$strict_excluded_member[[1L]]),
    relaxed_included_member = isTRUE(target$relaxed_included_member[[1L]]),
    relaxed_excluded_member = isTRUE(target$relaxed_excluded_member[[1L]]),
    identity_bridge_ready = isTRUE(target$identity_bridge_ready[[1L]]),
    target_chr_grch37 = target$reference_chr_grch37[[1L]],
    target_pos_grch37 = target$reference_pos_grch37[[1L]],
    ...
    source_marker_id = get_chr(row, "rs_number"),
    reference_allele = get_chr(row, "reference_allele"),
    other_allele = get_chr(row, "other_allele"),
    eaf = get_num(row, "eaf"),
    beta = get_num(row, "beta"),
    se = get_num(row, "se"),
    p_value = get_num(row, "p-value"),
    n_samples = get_num(row, "n_samples"),
    beta_95L = get_num(row, "beta_95L"),
    beta_95U = get_num(row, "beta_95U"),
    z = get_num(row, "z"),
    n_studies = get_num(row, "n_studies"),
    effects = get_chr(row, "effects"),
...
match_audit <- master[, c(
  "target_rsid", "expected_chen_marker_id", "raw_match_count", "match_status", "reason",
...
tmpdb <- rel("results", "tmp", "duckdb_chen_reverse_outcome_v1_recovery")
...
sql_path <- function(path) gsub("'", "''", norm(path), fixed = TRUE)
write_parquet_df <- function(x, path, table_name) {
  atomic_write(path, function(p) {
    ...
    DBI::dbExecute(con_db, sprintf("COPY %s TO '%s' (FORMAT PARQUET)", DBI::dbQuoteIdentifier(con_db, table_name), sql_path(p)))
  })
}
validate_pair <- function(parquet, tsv, key = "target_rsid") {
  p <- DBI::dbGetQuery(con_db, sprintf("SELECT * FROM read_parquet('%s')", sql_path(parquet)))
  ...
  numeric_stats <- lapply(num_cols, function(k) {
    ...
    data.frame(
      column = k,
      all_cells_ok = all(both_na | both_num),
  ...
  ns <- if (length(numeric_stats)) do.call(rbind, numeric_stats) else data.frame(column = character(), all_cells_ok = logical(), max_abs = numeric(), max_rel = numeric())
  numeric_ok <- all(ns$all_cells_ok) && all(ns$max_abs <= 1e-12 | ns$max_rel <= 1e-12)
...
hard_checks <- list(
  ...
  no_mr = TRUE,
  no_steiger = TRUE,
  failed_attempt_preserved = file.exists(paths$failed_log),
  technical_recovery_logged_separately = !identical(paths$failed_log, paths$recovery_log),
...
qc <- list(
  ...
  informational_findings = list(
    initial_failed_script = relpath(paths$failed_script),
    initial_failed_log = relpath(paths$failed_log),
    recovery_script = relpath(paths$recovery_script),
    recovery_log = relpath(paths$recovery_log),
    recovery_reason = "Initial Decision 110 attempt completed the raw scan but failed during R data.frame construction because a source-field accessor could return length zero; recovery uses explicit missing-field-to-NA accessors.",
...
decision_lines <- c(
  ...
  "The initial approved script completed a full raw source scan and preserved its failed log, but failed during R table construction because one source-field accessor could return length zero. This recovery script changes only the output-table accessor to return explicit NA for absent fields. It does not change target variants, matching rules, allele interpretation, or any scientific parameter.",
  "",
  "## Results",
  ...
  "",
  "## Branch Readiness",
  paste0("- strict_apoe_included: `", branch_counts$exact[branch_counts$branch == "strict_apoe_included"], "/", branch_counts$target[branch_counts$branch == "strict_apoe_included"], "` exact targets."),
  paste0("- strict_apoe_excluded: `", branch_counts$exact[branch_counts$branch == "strict_apoe_excluded"], "/", branch_counts$target[branch_counts$branch == "strict_apoe_excluded"], "` exact targets."),
  paste0("- relaxed_apoe_included: `", branch_counts$exact[branch_counts$branch == "relaxed_apoe_included"], "/", branch_counts$target[branch_counts$branch == "relaxed_apoe_included"], "` exact targets."),
  paste0("- relaxed_apoe_excluded: `", branch_counts$exact[branch_counts$branch == "relaxed_apoe_excluded"], "/", branch_counts$target[branch_counts$branch == "relaxed_apoe_excluded"], "` exact targets."),
  "",
  "## Outputs",
  paste0("- `", relpath(paths$master_parquet), "`"),
  paste0("- `", relpath(paths$master_tsv), "`"),
  paste0("- `", relpath(paths$match_audit), "`"),
  paste0("- `", relpath(paths$missing), "`"),
  paste0("- `", relpath(paths$qc_json), "`"),
  paste0("- preserved failed log: `", relpath(paths$failed_log), "`"),
  paste0("- recovery log: `", relpath(paths$recovery_log), "`")
)
